...
def add_wedge(ax, box, **kwargs):
    '''
    Given an Axes object (*ax*) and a tuple that defines the corners of the
    wedge as (min radius, max radius, min phi, max phi), add a wedge that spans
    the corners as given.  Extra keyword arguments are handed to *ax.plot*.
    '''
...
class Aurora(dict):
    ...
    def add_dial_plot(self, var, hemi='north', target=None, loc=111,
        ...
        *matplotlib.axes._subplots.AxesSubplot.contourf* method.
        ...
        cont = ax.pcolormesh(self.phi_mesh, self.colat_mesh, z, vmin=zlim[0],
                             cmap=cmap, vmax=zlim[-1], *args, norm=norm,
                             **kwargs)
    ...
    def mutate(self, hemi='north', yaw=0., pitch=0.0, roll=0.0, expand=0.):
        ...
        from numpy import pi, cos, sin, matmul
        from scipy.interpolate import LinearNDInterpolator as LinInt

        # If we have a feature, mutate that first:
        if hasattr(self, 'feature'):
            self.feature.mutate(hemi=hemi, yaw=yaw, pitch=pitch,
                                roll=roll, expand=expand)
    ...
    def set_feature_bounds(self, var='ilong'):
        '''
        Find the binding box and associated indices for the current location
        of the auroral feature.  Store for internal use.

        The feature is identified by the region where variable (*var*) crosses
        a threshold value (*sens*).

        The default behavior is to use "ilong" as the search variable and "0.1"
    ...
    def corr_feature(self, var, hemi='n', rect=False, noise=True,
                     flip=True, debug=False):
        '''
        Compute the cross-correlation matrix between a feature (e.g., arc or
        streamer) in one hemisphere with the entire oval in the opposite
        hemisphere.  This is essentially a test of feature identification
        between hemispheres.

        In other words, a template for pattern matching is taken from one
        hemisphere. The template is then cross-correlated with the opposite
        hemisphere.

        The section of the source hemisphere that is considered the template
        region is set automatically when a feature is loaded via
        *add_template*.
        ...
        position : tuple
            The longitude and colatitude of max_corr (mainly for plotting
            purposes).
        '''
        ...
        # Reduce template down to feature only.  Note that to overcome wraps
        # around phi=0, we use the roll function.  Set phimax to roll over
        # lon=0.
        phimax = loc[3]-loc[2]
        if phimax < 0:
            phimax = self.phi.size-phimax
