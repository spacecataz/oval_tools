...
fig.subplots_adjust(left=.04,right=.99,wspace=.11,hspace=.04,top=.92,bottom=.07)

# Some kwargs for brevity:
kwargs = {'zlim':[0,1500], 'target':fig}

### RAW OVALS + FEATURE
data = Aurora('data/AE_350_400.save')
...
### MUTATE
data.mutate('n', expand= 5, yaw= 5, roll= 5)
data.mutate('s', expand=-5, yaw=-5, roll=-5)
...
fig.text(text_x, out1[1].get_position().y1,
         'Step 3: Add Dayglow', ha='center', size=16)


### ADD NOISE
data.add_bright_noise('n', bkgd=50)
data.add_bright_noise('s', bkgd=50)
out1 = data.add_dial_plot('ishort', hemi='n', loc=427, **kwargs)
out2 = data.add_dial_plot('ishort', hemi='s', loc=428, **kwargs)
fig.text(text_x, out1[1].get_position().y1,
         'Step 4: Add Image Noise', ha='center', size=16)
